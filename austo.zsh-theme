...
prompt_context() {
  ...
  if [[ "$user" != "$branch" || -n "$SSH_CONNECTION" ]]; then
    prompt_segment $PRIMARY_FG default " %(!.%{%F{yellow}%}.)$user@%m "
  fi
}

# * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
# Fork from rberenguel/rb-zsh-theme https://github.com/rberenguel/rb-zsh-theme/blob/master/rb.zsh-theme
# * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
JIRA_PROJECT="APP"
JIRA_COMPANY="vaultinum"

shorten_git() {
    branch_ref=$1
    dashes=$(echo $branch_ref | tr -cd '-' | wc -c)
    slashes=$(echo $branch_ref | tr -cd '/' | wc -c)
    branch=$(echo $branch_ref | sed 's/\(.\{15\}\).*/\1.../')
    if [ "$slashes" -gt "$dashes" ]; then
        echo $branch | awk  'BEGIN{RS="/"; acum=""}!/^'${JIRA_PROJECT}'/{if(FNR==1){acum=$branch}else{acum=acum "/" $branch}} /^'$JIRA_PROJECT'/{acum=acum "/" $1 }END{print acum}'
    else
        echo $branch | awk  'BEGIN{RS="-"; acum=""}{if(FNR==1){acum=$branch}else{acum=acum "-" $branch}}END{print acum}'
    fi
}
